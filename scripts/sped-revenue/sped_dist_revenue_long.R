...
# updated 2026-08-27: added a schlev normalization step to all four
# year-blocks below. The FY22 raw file (sdf22_1a.csv) encodes schlev as a
# zero-padded code ("05","06","07") instead of the single-digit format every
# other year's file uses ("5","6","7"). The old filter (schlev != "5", etc.)
# silently failed to match the padded FY22 codes, so 270 non-district
# entities (county offices of education, vocational/special-ed systems, and
# other administrative/ESA units) were incorrectly left in the FY22 revenue
# sample instead of being excluded (confirmed against the raw FY22 file -
# corrected FY22 count is 12,710, not the 12,980 the unfixed filter
# produces). FY20, FY21, and FY23 were unaffected since those files already
# use unpadded codes, but the normalization is added to all four blocks so
# this keeps working correctly if a future data refresh pads any of them.
...
sped_revenue_fy20_clean <- sped_revenue_fy20_raw |>
  ...
  # This filters the districts to 12839
...
sped_revenue_fy22_clean <- sped_revenue_fy22_raw |>
  ...
  rename(district = name,
         ...
         federal_sped_rev = c15,
         total_enroll = membersch) |>
  # This year's raw file pads schlev with a leading zero ("05","06","07")
  # instead of the single-digit codes ("5","6","7") every other year uses.
  # Without normalizing it here, the schlev filters below silently match
  # nothing and this year's data set ends up 270 districts too large,
  # including entities like county offices of education that should be
  # excluded.
  mutate(schlev = str_remove(schlev, "^0+")) |>
  mutate(state_fed_sped_rev = state_sped_rev + federal_sped_rev) |>
  select(dist_id, state, district, schlev, agchrt, total_enroll, totalrev, tfedrev,
