...
# updated 2026-08-27:
#  (1) added a defensive schlev normalization step. This year's raw file
#      encodes schlev as a single digit ("5","6","7"), matching what the
...
#      correctly if a future refresh of the FY23 file is padded the same way.
#  (2) corrected the state count in the header note below: South Carolina
#      also has no districts with usable FY23 sped expenditure data (all its
#      districts are dropped by the current_sped_exp > 0 filter), so it's a
#      7th state missing from this data set, not a 6th.
