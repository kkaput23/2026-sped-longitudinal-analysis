# sped_district_expenditures_fy21
# last updated by Krista Kaput on 2026-02-18
# updated 2026-08-27: added a defensive schlev normalization step. This
# year's raw file encodes schlev as a single digit ("5","6","7"), matching
# what the filters below expect, so this is not fixing an active bug here -
# but the FY22 raw file turned out to use zero-padded codes ("05","06","07")
# instead, which silently broke the same filter logic in that year's script.
# Normalizing here too means this script keeps working correctly if a future
# refresh of the FY21 file is padded the same way.

# This script analyzes the FY21 special education expenditures
...
# There are 19,554 districts in the raw file
ccd_fy21_raw <- read_csv("sped-data/raw/sdf21_1a.csv")

# select the special education expenditure variables -------------

ccd_fy21_sped_expenditures <- ccd_fy21_raw |>
  rename_with(tolower) |>
...
ccd_sped_exp_clean <- ccd_fy21_sped_expenditures |>
  ...
  # This filters the districts to 9834
...
raw_summary <- ccd_fy21_sped_expenditures |>
  group_by(state) |>
...
write_csv(ccd_sped_exp_clean, "sped-data/processed/sped-expenditure/ccd_sped_expenditures_fy21.csv")


write_csv(state_total_district_count, "sped-data/processed/sped-expenditure/state_total_district_count_fy21.csv")


# Tidy workplace -----

rm(ccd_fy21_raw, ccd_fy21_sped_expenditures, ccd_fy21_sped_no_exp, sped_state_exp,
